...
.. _smarty-admin-guide-monitoring-triggers:

3.2.9.1. Триггеры
~~~~~~~~~~~~~~~~~

Данный раздел предназначен для настройки триггеров по определенным событиям мониторинга. На текущий момент существует 2 типа мониторинга:
* Мониторинг Multicast
...
* Ключи для мониторинга Multicast:
    ...
    * Потеря звука
    * Потеря видео
    * Поток недоступен (Not on air)
* Ключи для мониторинга устройств:
    * Средняя загрузка процессора
    * Температура процессора
    * Среднее время ожидания I/O
    * Использование ресурсов процессора приложением мониторинга
    * Использованная память
    * Свободная память
    * Всего памяти 
    * Использование памяти приложением мониторинга
    
При создании триггера необходимо выбрать один из ключей и установить для него крайние значения (для некоторых ключей установить значение нельзя, так как их значения не числовые, а логические - событие либо есть, либо его нет) - в случае попадания значения данного ключа в установленные рамки будет сгенерирована авария.
